...
- [func](https://github.com/knative-sandbox/kn-plugin-func/blob/decorate_namespace/docs/installing_cli.md)
...
```
pack build annotate-dev-cluster --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.0.10
```

Where `annotate-dev-cluster` is the name of your runnable function image, later used by Docker.
...
```
docker run -it --rm -p 8080:8080 annotate-dev-cluster
```
...
```
tanzu apps workload create annotate-dev-cluster -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/annotate-dev-cluster-source \
  --type web
```
...
```
tanzu apps workload get annotate-dev-cluster
```
